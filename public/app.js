...
const S = {
  ...
  reconnectTimer: null,
  authMode: 'login',
  previewMode: false,         // 预输入模式（半透明预览，确定后上传）
  preview: new Map(),         // 预输入预览层 "x,y" -> color
};
...
$('btnPreview').addEventListener('click', () => {
  ...
  $('btnPreview').classList.toggle('preview-on', S.previewMode);
  if (S.previewMode) { setMode('draw'); flashMode('预输入模式 · 半透明预览，确定后上传'); }
  else flashMode('已退出预输入模式');
  updatePreviewBar();
});
$('btnPvCommit').addEventListener('click', commitPreview);
$('btnPvDiscard').addEventListener('click', discardPreview);
...
function render() {
  ...
  // 预输入预览层：半透明显示待上传像素（与涂鸦同一缩放规则）
  if (S.preview.size && worldZoom(s) > HIDE_PIXELS_ZOOM) {
    ctx.save();
    ctx.globalAlpha = 0.5;
    for (const [k, c] of S.preview) {
      const i = k.indexOf(',');
      const cx = +k.slice(0, i), cy = +k.slice(i + 1);
      if (cx < x0 || cx > x1 || cy < y0 || cy > y1) continue;
      ctx.fillStyle = c;
...
function paintCell(x, y) {
  if (S.previewMode) return previewCell(x, y);
...
/* ---------- 预输入模式：半透明预览 + 实时扣点/恢复，确定后批量上传 ---------- */
function previewCell(x, y) {
  if (!requireLogin()) return;
  const key = cellKey(x, y);
  if (S.mode === 'erase') {
    // 擦除：仅撤销本地的预览像素，恢复 1 点
    if (!S.preview.has(key)) return;
    S.preview.delete(key);
    draw();
    savePreviewCache();
    apiPreviewRestore(1);
    return;
  }
  if (S.preview.get(key) === S.color) return;
  S.preview.set(key, S.color);
  draw();
  savePreviewCache();
  apiPreviewPlace(x, y, S.color);
}

// 预输入点数操作串行队列：避免并发请求响应乱序导致点数显示漂移
let _pvChain = Promise.resolve();

function apiPreviewPlace(x, y, color) {
  const key = cellKey(x, y);
  _pvChain = _pvChain.then(() =>
    api('/api/preview-place', { method: 'POST', body: { roomId: S.room?.id ?? null, x, y, color } })
  ).then((d) => {
    if (S.user) {
      S.user.points = d.points;
      S.user.pointLimit = d.limit;
...
function apiPreviewRestore(count) {
  _pvChain = _pvChain.then(() =>
    api('/api/preview-restore', { method: 'POST', body: { count } })
  ).then((d) => {
    if (S.user) { S.user.points = d.points; renderMe(); }
  }).catch((err) => { toast(err.message, true); });
  return _pvChain;
}

// localStorage 缓存预览操作（刷新/关闭页面不丢失，点确定后清空）
function savePreviewCache() {
  try {
    const ops = [...S.preview].map(([k, c]) => {
      const i = k.indexOf(',');
      return { x: +k.slice(0, i), y: +k.slice(i + 1), color: c };
    });
    localStorage.setItem('pv_preview', JSON.stringify({ ops }));
  } catch { /* ignore */ }
  updatePreviewBar();
}
function loadPreviewCache() {
  try {
    const d = JSON.parse(localStorage.getItem('pv_preview') || 'null');
    if (d && Array.isArray(d.ops) && d.ops.length) {
      d.ops.forEach((o) => S.preview.set(cellKey(o.x, o.y), o.color));
      toast(`检测到 ${d.ops.length} 格未上传的预输入像素，请点「确定上传」或「放弃」`);
    }
  } catch { /* ignore */ }
  updatePreviewBar();
  draw();
...
function updatePreviewBar() {
  const n = S.preview.size;
  $('pvCount').textContent = n;
  $('previewBar').classList.toggle('hidden', n === 0 && !S.previewMode);
  $('btnPvCommit').disabled = n === 0;
}

// 确定上传：批量落库 + 广播，成功后清空缓存
async function commitPreview() {
  if (!S.preview.size) return;
  try {
    const ops = [...S.preview].map(([k, c]) => {
      const i = k.indexOf(',');
      return { x: +k.slice(0, i), y: +k.slice(i + 1), color: c };
    });
    const d = await api('/api/commit-preview', { method: 'POST', body: { roomId: S.room?.id ?? null, ops } });
    S.preview.clear();
    localStorage.removeItem('pv_preview');
    S.previewMode = false;
    updatePreviewBar();
    draw();
    toast(`已上传 ${d.count} 格像素`, false);
  } catch (err) { toast(err.message, true); }
}
...
async function discardPreview() {
  const n = S.preview.size;
  if (!n) { S.preview.clear(); localStorage.removeItem('pv_preview'); updatePreviewBar(); draw(); return; }
  await apiPreviewRestore(n);   // 走串行链 + 更新前端点数
  S.preview.clear();
  localStorage.removeItem('pv_preview');
  S.previewMode = false;
  updatePreviewBar();
  draw();
  toast(`已放弃 ${n} 格预览，点数已恢复`);
}
